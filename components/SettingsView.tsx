...
import { getCerebrasKey, setCerebrasKey, getTavilyKey, setTavilyKey } from '../services/apiKey';
...
const SettingsView: React.FC = () => {
  const [cerebras, setCerebras] = useState(getCerebrasKey());
  ...
  const handleSaveAndTest = async () => {
    persist();
    if (!cerebras.trim()) {
      setStatus({ type: 'invalid', message: 'Ingresa tu API Key de Cerebras antes de probar.' });
      return;
    }
    setStatus({ type: 'validating' });
    const result = await validateApiKey(cerebras.trim());
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">Conecta Cerebras (IA) y Tavily (búsqueda web) para precios reales.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl space-y-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-gray-900">API Key de Cerebras <span className="text-red-500">*</span></h3>
          ...
          <div className="relative">
            <input
              ...
              value={cerebras}
              onChange={(e) => { setCerebras(e.target.value); setStatus({ type: 'idle' }); }}
              placeholder="csk-..."
            ...
            />
            <button type="button" onClick={() => setShowCerebras((s) => !s)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
              {showCerebras ? <EyeOff size={18} /> : <Eye size={18} />}
            </button>
          </div>
          <a href="https://cloud.cerebras.ai/" target="_blank" rel="noopener noreferrer"
            ...
            Conseguir una API Key de Cerebras (gratis) <ExternalLink size={14} />
          </a>
        </div>
        ...
        {status.type === 'validating' && (
          <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-lg">
            <Loader2 size={16} className="animate-spin" /> Validando con Cerebras...
          </div>
        )}
        {status.type === 'valid' && (
          <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 px-3 py-2 rounded-lg">
            <CheckCircle2 size={16} /> ¡Clave de Cerebras válida! Ya puedes usar la IA.
          </div>
        )}
        {status.type === 'invalid' && (
